...
gj_boundaries <- "C:\\Users\\Utente\\Desktop\\Research\\SolarPowerProductionPrediction\\Daily model on pes regions\\Data\\gb-dno-license-areas-20240503-as-geojson.geojson"
...
```{r capacity}
capacity_by_gsp <- read.csv("C:\\Users\\Utente\\Desktop\\Research\\SolarPowerProductionPrediction\\Hourly model on gsps\\Data\\capacity_by_gsp.csv")

gsp_location <- read.csv("C:\\Users\\Utente\\Desktop\\Research\\SolarPowerProductionPrediction\\Daily model on pes regions\\Data\\gsp_gnode_directconnect_region_lookup.csv")
...
weather_with_id <- st_join(weather_sf, boundaries_with_capacity[, c("ID")]) %>%
  ...
  summarise(avg_sun = mean(SUNSHINE_DUR), min_sun=min(SUNSHINE_DUR), max_sun=max(SUNSHINE_DUR), sun_sd = sd(SUNSHINE_DUR), .by=c(ID, OB_END_TIME)) #alternative is block kriging
```
...
weather_near_12 <- weather_sf %>%
  ...
  slice_head(n = 4) %>% 
  summarise(avg_sun = mean(SUNSHINE_DUR, na.rm = TRUE), min_sun = min(SUNSHINE_DUR), max_sun = max(SUNSHINE_DUR), sun_sd = sd(SUNSHINE_DUR), .groups = "drop") %>%
  mutate(ID = 12)  
...
weather <- bind_rows(weather_with_id, weather_near_12) %>%
  ...
  setNames(c("date", "pes_id", "avg_sun", "min_sun", "max_sun", "sun_sd")) %>%
  select(date, pes_id, avg_sun, min_sun, max_sun, sun_sd)
```
...
d_weather_2023_test <- read.csv("C:\\Users\\Utente\\Desktop\\Research\\SolarPowerProductionPrediction\\Daily model on pes regions\\Data\\daily_20_24\\midas_wxdrnl_202301-202312.txt", header=F)
colsnames <- c(
  "OB_END_TIME", "ID", "ID_TYPE", "OB_HOUR_COUNT", "VERSION_NUM", "MET_DOMAIN_NAME",
  "src_id", "REC_ST_IND", "CS_24HR_SUN_DUR", "CONC_STATE_ID", "LYING_SNOW_FLAG",
  "SNOW_DEPTH", "FRSH_SNOW_AMT", "SNOW_DAY_ID", "HAIL_DAY_ID", "THUNDER_DAY_FLAG",
...
test_weather <- bind_rows(test_weather_with_id, test_weather_near_12) %>%
  mutate(date = as.Date(OB_END_TIME), .before = ID) %>%
  arrange(date, ID) %>%
  select(date,ID,avg_sun, min_sun, max_sun, sun_sd) %>%
  setNames(c("date", "pes_id", "avg_sun", "min_sun", "max_sun", "sun_sd"))

test_gen_data <- read.csv("C:\\Users\\Utente\\Desktop\\Research\\SolarPowerProductionPrediction\\Daily model on pes regions\\Data\\test_daily_pv_generation.csv", header = T, sep = ",")
...
```{r eda2}
ggplot(data, aes(x = seq_along(avg_sun), y = avg_sun, color = as.factor(pes_id))) +
  geom_point(size = 0.5) +
  labs(color = "PES ID", y = "Sun Duration", x = "Index")
```
##### Plot of sunshine duration over the year 2024 for different regions. 
```{r eda3}
ggplot(data, aes(x = avg_sun, y = energy_mwh, color = as.factor(pes_id))) +
  geom_point(size = 0.5) +
...
- Region-level covariates (avg_sun, capacity) can be included as fixed effects.
...
rescaled_data <- data %>%
  mutate(
    sun_min_dev = min_sun - avg_sun,
    sun_max_dev = max_sun - avg_sun,
    avg_sun = scale(avg_sun),
    sun_sd = scale(sun_sd),
    sun_cv = scale(avg_sun/sun_sd),
    capacity_mwh = scale(capacity_mwh),
    min_sun = scale(min_sun),
    max_sun = scale(max_sun)
    )
...
```{r model1}
model1 <- lmer(
  log(energy_mwh) ~ avg_sun + capacity_mwh + 
    month +      
    (1 | pes_id),                    
  data = rescaled_data
)
...
model2 <- glmer(
  energy_mwh ~ avg_sun + capacity_mwh + 
    month +      
    (1 | pes_id),                    
  data = rescaled_data,
...
```{r multico}
cor(rescaled_data %>% 
      select(avg_sun, capacity_mwh, weekly_avg_energy_mwh, lag_energy_mwh, three_days_avg_energy_mwh), use = "complete.obs")
vif(lm(energy_mwh ~ avg_sun + capacity_mwh + month + three_days_avg_energy_mwh, 
       data = rescaled_data))
vif(lm(energy_mwh ~ avg_sun + capacity_mwh + month + weekly_avg_energy_mwh, 
       data = rescaled_data))
```
...
for (pes in 10:23) {
  ...
  hist(
    ...
    breaks = 30,
    col = "purple",
    main = paste("Box-Cox (λ=", round(best_lambda, 2), ") Region", pes),
...
predicted_all <- bind_cols(bind_rows(lapply(results_normal, `[[`, "data")),bind_rows(lapply(results_gamma, `[[`, "data")), ) %>% select(date...1, pes_id...2, energy_mwh...3,regional_predicted_energy...24, regional_predicted_energy...48)
...
```{r plolplo}
ggplot(predicted_all, aes(x = energy_mwh...3, y = regional_predicted_energy...24, color = as.factor(pes_id...2))) +
  geom_point(alpha = 0.6, size = 1.5) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "gray") +
...
ggplot(predicted_all, aes(x = energy_mwh...3, y = regional_predicted_energy...48, color = as.factor(pes_id...2))) +
  geom_point(alpha = 0.6, size = 1.5) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "gray") +
...
### Adding interaction between avg_sun and month in the model with all the regions to see if it captures the differences arisen in the regional models, and probably due to sun inclination differences throughout the region.

```{r interactions}
model67_interacted <- glmer(
  energy_mwh ~ avg_sun*month + capacity_mwh +       
    (1 | pes_id),
    data = rescaled_data,
  family = Gamma(link = "log")
)
...
### We still want to improve the predictions using the GLMM. 

#### This means that we want to account for that overestimation in our model. First, we cluster the outliers to find some common patterns in them. Since we have different scales in the regions, we want to cluster outliers based on the scale of the region. This is because a residual of 300 in region 12 has a very different meaning from a residual of 300 in region 14.

```{r outliers}
rescaled_test_data$residuals <- rescaled_test_data$energy_mwh - rescaled_test_data$predicted_energy_67_interacted

rescaled_test_data <- rescaled_test_data %>%
  mutate(area = ifelse(pes_id %in% c(15,16,17,18), "North", "South")) %>%
  group_by(pes_id)  %>%
  mutate(res_mean = mean(residuals),
         res_sd = sd(residuals),
         std_residuals = (residuals - res_mean)/res_sd) %>%
  ungroup() %>%
  mutate(is_outlier = ifelse(abs(std_residuals)>2,TRUE,FALSE)) %>%
  select(-predicted_energy_5,-predicted_energy_6,-predicted_energy_56,-predicted_energy_67,-lag_energy_mwh,-weekly_avg_energy_mwh)

rescaled_test_data %>%
  ggplot(aes(x = energy_mwh, y = predicted_energy_67_interacted, color = is_outlier)) +
  geom_point(alpha = 0.6, size = 1.5) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "gray") +
  facet_wrap(~ area) +
  labs(title = "Model 67_interacted: Predicted vs Observed Energy Generation",
...
#### We observe that most outliers happen for overestimation of large values. There are also outliers for smaller scales which we must consider.

#### From a temporal point of view, we are interested in the spatial and temporal correlations of outliers.
...
rescaled_test_data %>%
  ...
  ungroup()
```
#### We can assume that all regions have the same number of outliers and that these differences are random.
...
#### We notice that there are definitely some month with more outliers (May, June, September), which seem to correspond to spring and autumn seasons. Could this be due to more weather uncertainty inntra-day in the same region?
...
rescaled_test_data %>%
  ...
  arrange(area)
```
#### This confirms what we said about spring and autumn months. We also notice an unusual spike for outliers in February in the South.

#### Let's now plot the distribution in time of the outliers.

```{r out3}
outlier_counts <- rescaled_test_data %>%
  filter(is_outlier == 1) %>%
  group_by(date) %>%
  summarise(n_outliers = n())
outlier_counts

ggplot(rescaled_test_data, aes(x = date, y = predicted_energy_67_interacted)) +
  geom_line(aes(group = pes_id), color = "gray80", alpha = 0.4) +
  geom_point(data = outlier_counts, aes(x = date, y = max(rescaled_test_data$predicted_energy_67_interacted), size = n_outliers),
             color = "red", alpha = 0.8, inherit.aes = FALSE) +
  scale_size_continuous(range = c(1, 5)) +
  labs(
    title = "Predicted Energy Over Time with Outlier Frequency Overlay",
    x = "Date",
    y = "Predicted energy_mwh",
    size = "Outlier Count"
  ) +
...
ggplot(outlier_counts, aes(x = date, y = n_outliers)) +
  geom_point(aes(size = n_outliers), color = "red", alpha = 0.7) +
  scale_size_continuous(range = c(2, 8)) +
  labs(
    title = "Number of Regional Outliers per Day",
    x = "Date",
  ...
  theme_minimal()
```
#### We notice from the two plots that many outliers actually fall on the same day. For example, on 05/29 and on 05/30 the model predicts 6 outliers (so 12 predictions out of 28 are outliers) each, while in the following 4 days only 4 predictions out of 56 are outliers. This is a very interesting behavior that we want to understand better. We hypothesize that this may be due to the regional estimation of daily sunshine which could be very biased as regions are large and weather can be very variable in the region.

#### Let's look at weekly clustering of outliers, to see if there is an even more remarkable trend.

```{r out4}
rescaled_test_data <- rescaled_test_data %>%
  mutate(week = paste0(year(date), "-W", sprintf("%02d", isoweek(date))))
...
ggplot(outlier_counts, aes(x = week, y = n_outliers)) +
  geom_point(aes(size = n_outliers), color = "red", alpha = 0.7) +
  scale_size_continuous(range = c(2, 8)) +
  labs(
    title = "Number of Regional Outliers per Week",
    x = "Date",
    y = "Number of Outliers",
    size = "Outlier Count"
  ) +
...
#### This shows once again that outliers are likely to happen on similar days throughout all the regions. Some weeks count over 30 outliers. Our model is missing something.

### Adding info about sunshine distribution in each region

#### We want to account for sunshine duration's distribution in the model, as at the moment we only have the mean to describe it. We want to find the mininum and maximum value for every region, as well as their deviation from the mean, the standard deviation within each region for each day and the coefficient of variation (sd/mean). We also want to use weather stations with missing data to make our estimates more reliable.

#### After transformation, this is what our dataset now looks like:

```{r weather_distribution}
head(rescaled_test_data)
```

#### We are interested in learning if the outlier status is correlated with higher variability in the sunshine duration. For this reason, we check the distribution of outliers and non-outliers as well as the averages for the two groups's standard deviations. We also perform a Wilcox test for non-Gaussian data for paired samples (H0 : distributions are the same). We fail to reject the null-hypothesis. There is no systematic difference between standard deviations on days with outliers and days without outliers.

```{r weather_distribution2}
rescaled_test_data %>%
  group_by(is_outlier) %>%
  summarise(
    mean_sd = mean(sun_sd, na.rm = TRUE),
    median_sd = median(sun_sd, na.rm = TRUE),
    count = n()
  )

rescaled_test_data %>%
  ggplot(aes(x = sun_sd, fill = as.factor(is_outlier))) +
  geom_density(alpha = 0.5) +
  labs(title = "Sunshine Variability vs Outlier Status",
       x = "Standard Deviation of Sunshine Duration (sun_sd)",
       fill = "Outlier")


wilcox.test(sun_sd ~ is_outlier, data = rescaled_test_data)
```


```{r distr329}
avg_sd_per_day <- rescaled_test_data %>%
  group_by(date) %>%
  summarise(avg_sd = mean(sun_sd, na.rm = TRUE), .groups = "drop")

outlier_counts <- rescaled_test_data %>%
  filter(is_outlier == TRUE) %>%
  count(date, name = "n_outliers")

plot_data <- left_join(avg_sd_per_day, outlier_counts, by = "date") %>%
  mutate(n_outliers = replace_na(n_outliers, 0))  # replace NA with 0

ggplot(plot_data, aes(x = date, y = avg_sd)) +
  geom_line(color = "steelblue", linewidth = 1) +
  geom_point(aes(size = n_outliers), color = "red", alpha = 0.8) +
  scale_size_continuous(range = c(1, 6)) +
  labs(
    title = "Average Sunshine Duration Variability and Outlier Counts Over Time",
    x = "Date",
    y = "Average Sunshine Duration SD",
    size = "Outlier Count"
  ) +
  theme_minimal()
```

#### We have previously said that outlier happen more often on spring and autumn days. Looking at this plot, we may assume that those days have on average higher weather uncertainty leading to higher standard deviations in sunshine duration, which ultimately coincide with days with the most outliers.

#### We want to check via linear and logistic regression the correlation between sunshine duration standard deviation and outlier status. The predictor is not significant but it does have a positive estimate. This could be only correlation and no causation effect.

```{r modelsforsd}
sd_outlier_model <- lm(sun_sd~is_outlier, rescaled_test_data)
summary(sd_outlier_model)

ggplot(rescaled_test_data, aes(x = date, y = sun_sd)) +
  geom_line(color = "steelblue", linewidth = 0.5) +
  geom_point(data = rescaled_test_data %>% filter(is_outlier == TRUE),
    aes(x = date, y = sun_sd),
  ...
  facet_wrap(~ pes_id) +
  labs(
    title = "Average Sunshine Duration Variability and Outlier Counts Over Time per Region",
    x = "Date",
    y = "Average Sunshine Duration SD",
    size = "Outlier Count"
  ) +
...
```
#### This plot could be the final one showing that the correlation between sunshine duration standard deviation and outlying prediction is vague and negligible. We still try to fit a new model with distributional information added to see the effects.

#### We also add more refined capacity data.

```{r capacitynew}
capacity_mwh <- data.frame(
  pes_id = as.character(10:23),
  capacity_mwh = c(3.4e+3,3.0e+3, 171, 767, 1.32e+3, 618, 640, 309, 448, 1.06e+3, 3.22e+3, 1.24e+3, 2.33e+3, 914)
)

rescaled_data <- rescaled_data %>%
  select(!capacity_mwh) %>%
  right_join(capacity_mwh, by="pes_id") %>%
  mutate(capacity_mwh=scale(capacity_mwh),
         sun_min_dev = scale(sun_min_dev),
         sun_max_dev = scale(sun_max_dev))
...
```{r modelwithdistr}
model67_interacted_distribution <- glmer(
  energy_mwh ~ avg_sun*month + sun_sd + capacity_mwh + 
    (1 | pes_id),
    data = rescaled_data,
  family = Gamma(link = "log")
)

summary(model67_interacted_distribution)
paste("AIC: ", AIC_model67_interacted_distribution <- extractAIC(model67_interacted_distribution)[2])

rescaled_test_data$predicted_energy_67_interacted_distribution <- predict(
  model67_interacted_distribution,
  newdata = rescaled_test_data,
  type = "response",         
  allow.new.levels = FALSE   
)

rmse_score_67_interacted_distribution <- rmse(rescaled_test_data$energy_mwh, rescaled_test_data$predicted_energy_67_interacted_distribution)
mae_score_67_interacted_distribution <- mae(rescaled_test_data$energy_mwh, rescaled_test_data$predicted_energy_67_interacted_distribution)

cat("Model 67_interacted_distribution -> RMSE:", rmse_score_67_interacted_distribution, "\nMAE:", mae_score_67_interacted_distribution)

ggplot(rescaled_test_data, aes(x = energy_mwh, y = predicted_energy_67_interacted_distribution, color = as.factor(pes_id))) +
  geom_point(alpha = 0.6, size = 1.5) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "gray") +
  labs(title = "Model 67_interacted: Predicted vs Observed Energy Generation",
       x = "Observed energy_mwh",
...
residuals_67_interacted_distribution <- pull(rescaled_test_data, energy_mwh) - pull(rescaled_test_data, predicted_energy_67_interacted_distribution)

plot_df <- data.frame(predictions = pull(rescaled_test_data, predicted_energy_67_interacted_distribution), residuals = residuals_67_interacted_distribution, pes_id = pull(rescaled_test_data, pes_id)) %>%
  mutate(area = ifelse(pes_id %in% c(15,16,17,18), "North", "South"))
...
# Define a custom plotting function so style is consistent
plot_pred_vs_obs <- function(data, y_var, title_text) {
  ggplot(data, aes(x = energy_mwh, y = .data[[y_var]], color = as.factor(pes_id))) +
    geom_point(alpha = 0.6, size = 1.5) +
...
p1 <- plot_pred_vs_obs(rescaled_test_data, "predicted_energy_2", "No interaction and no sd")
p2 <- plot_pred_vs_obs(rescaled_test_data, "predicted_energy_67_interacted", "Only interaction")
p3 <- plot_pred_vs_obs(rescaled_test_data, "predicted_energy_67_interacted_distribution", "Both interaction and sd")

# Combine side by side
combined_plot <- p1 + p2 + p3 + plot_layout(guides = "collect")

# Save to PDF
ggsave("predicted_vs_observed_models.pdf", combined_plot, width = 12, height = 4)
```

#### With standard deviation of the sunshine duration added to the model, we do get a slightly better model (5.5$ improvement on MAE, 8% improvement on RMSE).

#### We also try adding deviations.

```{r modelwithdistr}
model67_interacted_distribution2 <- glmer(
  energy_mwh ~ avg_sun*month + capacity_mwh + sun_max_dev*sun_min_dev + 
    (1 | pes_id),
    data = rescaled_data,
  family = Gamma(link = "log")
)

summary(model67_interacted_distribution2)
paste("AIC: ", AIC_model67_interacted_distribution2 <- extractAIC(model67_interacted_distribution2)[2])

rescaled_test_data$predicted_energy_67_interacted_distribution2 <- predict(
  model67_interacted_distribution2,
  newdata = rescaled_test_data,
  type = "response",         
  allow.new.levels = FALSE   
)

rmse_score_67_interacted_distribution2 <- rmse(rescaled_test_data$energy_mwh, rescaled_test_data$predicted_energy_67_interacted_distribution2)
mae_score_67_interacted_distribution2 <- mae(rescaled_test_data$energy_mwh, rescaled_test_data$predicted_energy_67_interacted_distribution2)

cat("Model 67_interacted_distribution2 -> RMSE:", rmse_score_67_interacted_distribution2, "\nMAE:", mae_score_67_interacted_distribution2)

ggplot(rescaled_test_data, aes(x = energy_mwh, y = predicted_energy_67_interacted_distribution2, color = as.factor(pes_id))) +
  geom_point(alpha = 0.6, size = 1.5) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "gray") +
  labs(title = "Model 67_interacted_distribution2: Predicted vs Observed Energy Generation",
       x = "Observed energy_mwh",
       y = "Predicted energy_mwh",
       color = "PES ID") +
  theme_minimal()

residuals_67_interacted_distribution2 <- pull(rescaled_test_data, energy_mwh) - pull(rescaled_test_data, predicted_energy_67_interacted_distribution2)

plot_df <- data.frame(predictions = pull(rescaled_test_data, predicted_energy_67_interacted_distribution2), residuals = residuals_67_interacted_distribution2, pes_id = pull(rescaled_test_data, pes_id)) %>%
  mutate(area = ifelse(pes_id %in% c(15,16,17,18), "North", "South"))
...
## Using 3 years of data to train model and 1 year to test:
### 2020-2023 train set / 2024 test set

```{r big_data, include=F, echo=F}
colsnames <- c(
  "OB_END_TIME", "ID", "ID_TYPE", "OB_HOUR_COUNT", "VERSION_NUM", "MET_DOMAIN_NAME",
  "src_id", "REC_ST_IND", "CS_24HR_SUN_DUR", "CONC_STATE_ID", "LYING_SNOW_FLAG",
  "SNOW_DEPTH", "FRSH_SNOW_AMT", "SNOW_DAY_ID", "HAIL_DAY_ID", "THUNDER_DAY_FLAG",
...
d_weather_2020 <- read.csv("C:\\Users\\Utente\\Desktop\\Research\\data\\daily_20_24\\midas_wxdrnl_202001-202012.txt", header=F)
d_weather_2021 <- read.csv("C:\\Users\\Utente\\Desktop\\Research\\data\\daily_20_24\\midas_wxdrnl_202101-202112.txt", header=F)
d_weather_2022 <- read.csv("C:\\Users\\Utente\\Desktop\\Research\\data\\daily_20_24\\midas_wxdrnl_202201-202212.txt", header=F)
d_weather_2023 <- read.csv("C:\\Users\\Utente\\Desktop\\Research\\data\\daily_20_24\\midas_wxdrnl_202301-202312.txt", header=F)
d_weather_2024 <- read.csv("C:\\Users\\Utente\\Desktop\\Research\\data\\daily_20_24\\midas_wxdrnl_202401-202412.txt", header=F)

colnames(d_weather_2020) <- colsnames
colnames(d_weather_2021) <- colsnames
colnames(d_weather_2022) <- colsnames
colnames(d_weather_2023) <- colsnames
colnames(d_weather_2024) <- colsnames

d_weather_2020 <- d_weather_2020 %>%
  select(-THUNDER_DAY_FLAG)
d_weather_2021 <- d_weather_2021 %>%
  select(-THUNDER_DAY_FLAG)
d_weather_2022 <- d_weather_2022 %>%
  select(-THUNDER_DAY_FLAG)
d_weather_2023 <- d_weather_2023 %>%
  select(-THUNDER_DAY_FLAG)
d_weather_2024<- d_weather_2024 %>%
  select(-THUNDER_DAY_FLAG)

all_years <- bind_rows(d_weather_2020,
                       d_weather_2021,
                       d_weather_2022,
                       d_weather_2023,
                       d_weather_2024) 

test_weather <- all_years %>%
  left_join(all_stations, by="src_id")
...
test_gen_data <- read.csv("C:\\Users\\Utente\\Desktop\\Research\\SolarPowerProductionPrediction\\20_24_daily_pv_generation.csv", header = T, sep = ",")
test_gen_data$pes_id <- as.character(test_gen_data$pes_id)
test_gen_data$date <- as.Date(test_gen_data$date)
...
data <- test_gen_data %>%
  ...
  left_join(capacity_by_pes, by="pes_id") %>%
  mutate(month = format(date, "%m")) #assuming no effect between years (probably there is though)
data$month <- as.factor(test_data$month)
...
data <- data %>%
  mutate(
    ...
    min_sun = scale(min_sun),
    max_sun = scale(max_sun),
    energy_mwh = ifelse(energy_mwh == 0, 0.001, energy_mwh)
    ) 
...
best_model_with_more_data <- glmer(
  energy_mwh ~ avg_sun*month + capacity_mwh + sun_sd + 
    (1 | pes_id),
    data = train_set,
  family = Gamma(link = "log")
)

summary(best_model_with_more_data)
paste("AIC: ", AIC_best_model <- extractAIC(best_model_with_more_data)[2])

test_set$predictions <- predict(
  best_model_with_more_data,
  newdata = test_set,
  type = "response",         
  allow.new.levels = FALSE   
)

rmse_score_best <- rmse(test_set$energy_mwh, test_set$predictions)
mae_score_best <- mae(test_set$energy_mwh, test_set$predictions)

cat("Best Model -> RMSE:", rmse_score_best, "\nMAE:", mae_score_best)

ggplot(test_set, aes(x = energy_mwh, y = predictions, color = as.factor(pes_id))) +
  geom_point(alpha = 0.6, size = 1.5) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "gray") +
  labs(title = "Model with more data: Predicted vs Observed Energy Generation",
       x = "Observed energy_mwh",
       y = "Predicted energy_mwh",
       color = "PES ID") +
  theme_minimal()

test_set$residuals <- pull(test_set, energy_mwh) - pull(test_set, predictions)

plot_df <- data.frame(predictions = pull(test_set, predictions), residuals = residuals, pes_id = pull(test_set, pes_id)) %>%
  mutate(area = ifelse(pes_id %in% c(15,16,17,18), "North", "South"))
...
test_set %>%
  ggplot(aes(x = energy_mwh, y = predictions, color = is_outlier)) +
  geom_point(alpha = 0.6, size = 1.5) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "gray") +
  facet_wrap(~ area) +
  labs(title = "Model 67_interacted: Predicted vs Observed Energy Generation",
...
ggplot(test_set, aes(x = date, y = predictions)) +
  geom_line(aes(group = pes_id), color = "gray80", alpha = 0.4) +
  geom_point(data = outlier_counts, aes(x = date, y = max(test_set$predictions), size = n_outliers),
             color = "red", alpha = 0.8, inherit.aes = FALSE) +
  scale_size_continuous(range = c(1, 5)) +
  labs(
    title = "Predicted Energy Over Time with Outlier Frequency Overlay",
...
#### As more data is added, the model now tends to UNDERESTIMATE: weird behavior. Also, the MAE and RMSE increase meaning previous results may be due to randomness. Even when training with more data, our outliers tend to happen mainly in the summer months.

#### We may need more training and test data to have more sure results.
...
stidf <- STIDF(
  ...
  data = rescaled_data[, c("energy_mwh", "avg_sun", "capacity_mwh")]
)
...
```{r var}
variog_formula <- energy_mwh ~ avg_sun + capacity_mwh

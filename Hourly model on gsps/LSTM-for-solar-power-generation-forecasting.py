...
x_1 = LSTM(64, return_sequences=True, dropout = 0.2, recurrent_dropout=0)(seq_inp)
x_2 = LSTM(32, return_sequences=False, dropout = 0.2, recurrent_dropout=0)(x_1) 
...
w = Dense(32, activation="relu")(w)  # learn transformation
...
pred_df.to_csv("predictions_199.csv", index=False)

metrics = {"MAE": mae, "RMSE": rmse}
metrics_df = pd.DataFrame([metrics])
metrics_df.to_csv("metrics_199.csv", index=False)
...
# --- 1. Full test set predictions vs truth ---
plt.figure(figsize=(15,5))
plt.plot(test_dates[n_past:], y_true.flatten(), label="True", alpha=0.7)
plt.plot(test_dates[n_past:], y_pred.flatten(), label="Predicted", alpha=0.7)
plt.title("Predictions vs True Values - Full Test Set")
plt.xlabel("Time")
plt.ylabel("Generation (MW)")
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(diag_dir, "full_test.png"))
plt.close()


# --- 2. Example: specific day ---
def plot_day(day_str):
    """day_str format: 'YYYY-MM-DD'"""
    mask = test_dates.dt.date == pd.to_datetime(day_str).date()
    if mask.sum() == 0:
        print(f"No test data for {day_str}")
        return
    plt.figure(figsize=(12,4))
    plt.plot(test_dates[n_past:][mask[n_past:]], y_true.flatten()[mask[n_past:]], label="True")
    plt.plot(test_dates[n_past:][mask[n_past:]], y_pred.flatten()[mask[n_past:]], label="Predicted")
    plt.title(f"Predictions vs True - Day {day_str}")
    plt.xlabel("Time (hour)")
    plt.ylabel("Generation (MW)")
    plt.legend()
    plt.tight_layout()
    fname = f"day_{day_str}.png".replace(":", "-")
    plt.savefig(os.path.join(diag_dir, fname))
    plt.close()


# --- 3. Example: specific week ---
def plot_week(start_day_str):
    """start_day_str format: 'YYYY-MM-DD'"""
    start_date = pd.to_datetime(start_day_str)
    end_date = start_date + pd.Timedelta(days=7)
    mask = (test_dates >= start_date) & (test_dates < end_date)
    if mask.sum() == 0:
        print(f"No test data for week starting {start_day_str}")
        return
    plt.figure(figsize=(15,5))
    plt.plot(test_dates[n_past:][mask[n_past:]], y_true.flatten()[mask[n_past:]], label="True")
    plt.plot(test_dates[n_past:][mask[n_past:]], y_pred.flatten()[mask[n_past:]], label="Predicted")
    plt.title(f"Predictions vs True - Week of {start_day_str}")
    plt.xlabel("Time")
    plt.ylabel("Generation (MW)")
    plt.legend()
    plt.tight_layout()
    fname = f"week_{start_day_str}.png".replace(":", "-")
    plt.savefig(os.path.join(diag_dir, fname))
    plt.close()

# Full test set plot is already saved

# Pick a few example dates/weeks you want to zoom into
plot_day("2022-06-15")    # one sunny day
plot_day("2022-12-01")    # a winter day
plot_week("2022-03-01")   # spring week
plot_week("2022-10-01")   # autumn week
...
from plotnine import ggsave as gg_save
...
p_loss = (
    ...
    labs(title="Training and Validation Loss for GSP 199",
...
# Get matplotlib figure from plotnine plot
fig = p_loss.draw()

# Save directly with matplotlib
fig.savefig(
    "fig10_loss_199.png",
    dpi=300,
    bbox_inches="tight"
